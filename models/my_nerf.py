...
class MyNeRF():
    ...
    def save(self, pts_xyz, sigma, color):
        ...
        try:
            ...
            print('VALID CHECKPOINT FOUND', self.volumes_sigma.shape, self.volumes_color.shape)
            return
        except:
            # 存储体积中各点的sigma和color
            ...
            print('NEW CHECKPOINT CREATED', self.volumes_sigma.shape, self.volumes_color.shape)
    ...
    def query(self, pts_xyz):
        ...
        # 获取待查询坐标的x,y,z分量
        X_index = ((pts_xyz[:, 0] + 0.125) * 4 * self.N).clamp(0, self.N-1).long()
        Y_index = ((pts_xyz[:, 1] - 0.75) * 4 * self.N).clamp(0, self.N-1).long()
        Z_index = ((pts_xyz[:, 2] + 0.125) * 4 * self.N).clamp(0, self.N-1).long()
